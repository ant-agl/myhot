...
if(isset($_GET['search']) and isset($_GET['input_date']) and isset($_GET['output_date']) and is_numeric($_GET['input_date']) and is_numeric($_GET['output_date'])){
	...
	$sqls = "SELECT * FROM `reserve` WHERE not input_date range({$_GET['input_date']},".($_GET['output_date']-1).") and not output_date range (".($_GET['input_date']+1).",{$_GET['output_date']}) INNER JOIN ($sql) ON reserve.id_hotel = hotel_search.id INNER JOIN (";
