...
if ($user_auth_data) {
    ...
    $sql = "SELECT `date`, `opinion`, `rating` FROM reviews WHERE uid = '".$id."' INNER JOIN (SELECT `input_date`, `output_date` FROM reserve) ON reviews.hotel_id = reserve.id INNER JOIN (SELECT `name`, `city`, `country`, `image`, `rating.stars` FROM hotel_search) ON reviews.hotel_id = hotel_search.id INNER JOIN (SELECT `name` FROM rooms_search) ON reviews.room_id = rooms_search.id";
