...
if ($user_auth_data) {
    ...
    $balance = "SELECT `balance` FROM users WHERE id = ".$id;
    $sql = "SELECT `input_date`, `output_date`, `cost_full`, `bonus` FROM reserve INNER JOIN (SELECT `name`, `city`, `country`, `image` FROM hotel_search) ON reserve.id_hotel = hotel_search.id WHERE uid = ".$id;
	...
	curl_close($ch);
    echo json_encode(array('history' => $result, 'balance' => $result2[0]['balance']), JSON_UNESCAPED_UNICODE);
}
?>